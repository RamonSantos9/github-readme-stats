...
describe("Test fetchStats", () => {
  it("should fetch correct stats", async () => {
    let stats = await fetchStats("ramonsantos9");
  ...
  it("should fetch total commits", async () => {
    ...
    let stats = await fetchStats("ramonsantos9", true);
    const rank = calculateRank({
      all_commits: true,
      commits: 1000,
      prs: 300,
  ...
  it("should throw specific error when include_all_commits true and API returns error", async () => {
    ...
    expect(fetchStats("ramonsantos9", true)).rejects.toThrow(
  ...
  it("should exclude stars of the `test-repo-1` repository", async () => {
    ...
    let stats = await fetchStats("ramonsantos9", true, ["test-repo-1"]);
    const rank = calculateRank({
      all_commits: true,
      commits: 1000,
      prs: 300,
  ...
  it("should fetch additional stats when it requested", async () => {
    let stats = await fetchStats("ramonsantos9", false, [], true, true, true);
    const rank = calculateRank({
      all_commits: false,
      commits: 100,
      prs: 300,
