import { afterEach, describe, expect, it } from "@jest/globals";
import "@testing-library/jest-dom";
import axios from "axios";
import MockAdapter from "axios-mock-adapter";
import { fetchGist } from "../src/fetchers/gist.js";

const gist_data = {
  data: {
    viewer: {
      gist: {
        description:
          "List of countries and territories in English and Spanish: name, continent, capital, dial code, country codes, TLD, and area in sq km. Lista de países y territorios en Inglés y Español: nombre, continente, capital, código de teléfono, códigos de país, domínio e área em km quadrados. Atualizado em 2023",
        owner: {
          login: "Yizack",
        },
        stargazerCount: 33,
        forks: {
          totalCount: 11,
        },
        files: [
          {
            name: "countries.json",
            language: {
              name: "JSON",
            },
            size: 85858,
          },
          {
            name: "territories.txt",
            language: {
              name: "Text",
            },
            size: 87858,
          },
          {
            name: "countries_spanish.json",
            language: {
              name: "JSON",
            },
            size: 85858,
          },
          {
            name: "territories_spanish.txt",
            language: {
              name: "Text",
            },
            size: 87858,
          },
        ],
      },
    },
  },
};

const gist_not_found_data = {
  data: {
    viewer: {
      gist: null,
    },
  },
};

const gist_errors_data = {
  errors: [
    {
      message: "Erro de teste do GraphQL",
    },
  ],
};

const mock = new MockAdapter(axios);

afterEach(() => {
  mock.reset();
});

describe("Testes em fetchers/gist.ts", () => {
  it("deve buscar o gist corretamente", async () => {
    mock.onPost("https://api.github.com/graphql").reply(200, gist_data);

    const gist = await fetchGist("bbfce31e0217a3689c8d961a356cb10d");

    expect(gist).toStrictEqual({
      name: "countries.json",
      nameWithOwner: "Yizack/countries.json",
      description:
        "List of countries and territories in English and Spanish: name, continent, capital, dial code, country codes, TLD, and area in sq km. Lista de países y territorios en Inglés y Español: nombre, continente, capital, código de teléfono, códigos de país, domínio e área em km quadrados. Atualizado em 2023",
      language: "Text",
      starsCount: 33,
      forksCount: 11,
    });
  });

  it("deve lançar o erro correto se o gist não for encontrado", async () => {
    mock
      .onPost("https://api.github.com/graphql")
      .reply(200, gist_not_found_data);

    await expect(fetchGist("bbfce31e0217a3689c8d961a356cb10d")).rejects.toThrow(
      "Gist não encontrado",
    );
  });

  it("deve lançar erro se a resposta contiver erros", async () => {
    mock.onPost("https://api.github.com/graphql").reply(200, gist_errors_data);

    await expect(fetchGist("bbfce31e0217a3689c8d961a356cb10d")).rejects.toThrow(
      "Erro de teste do GraphQL",
    );
  });

  it("deve lançar erro se o id não for fornecido", async () => {
    await expect(fetchGist("")).rejects.toThrow(
      'Estão faltando parâmetros "id" certifique-se de passar os parâmetros na URL',
    );
  });
});
