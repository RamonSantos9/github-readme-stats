...
describe("Testes em fetchers/wakatime.ts", () => {
  it("deve buscar os dados do WakaTime corretamente", async () => {
    const username = "ramonsantos9";
    mock
      .onGet(
  ...
  it("deve lançar erro se o parâmetro username estiver faltando", async () => {
    // @ts-ignore: teste intencional de erro de parâmetro
    await expect(fetchWakatimeStats({})).rejects.toThrow(
      'Estão faltando parâmetros "username" certifique-se de passar os parâmetros na URL',
    );
  });
