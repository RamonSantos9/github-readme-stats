/**
 * @fileoverview Módulo de localização (internacionalização) do projeto.
 *
 * @description
 * Contém as traduções para todas as strings exibidas nos cards do GitHub README Stats,
 * organizadas por componente (statCard, repoCard, langCard, wakatimeCard).
 * Cada chave de tradução mapeia para um objeto com os idiomas suportados.
 *
...
/** Tipo para um mapa de localização: chave de idioma → string traduzida. */
type LocaleMap = Record<string, string>;

/** Tipo para um conjunto de traduções: chave de tradução → LocaleMap. */
type LocaleSet = Record<string, LocaleMap>;

/**
 * Retorna as traduções do card de estatísticas em todos os idiomas disponíveis.
 *
 * @param props - Argumentos da função.
 * @param props.name - Nome do usuário.
 * @param props.apostrophe - Apóstrofo a ser usado conforme o idioma.
 * @returns Objeto com todas as traduções do card de estatísticas.
 *
 * @see https://www.andiamo.co.uk/resources/iso-language-codes/ para códigos de idioma.
 */
...
/**
 * Retorna as traduções do card de sequências (streaks).
 *
 * @returns Objeto com todas as traduções do card de streaks.
 */
const streakCardLocales = (): LocaleSet => {
  return {
    "streakcard.total": {
...
/**
 * Verifica se o locale fornecido está disponível.
 *
 * @param locale - O código do locale a verificar (ex: "pt-br").
 * @returns `true` se o locale estiver disponível, `false` caso contrário.
 */
...
const activityGraphLocales = ({
  ...
  return {
    "activitygraph.title": {
      ...
      "pt-br": `Gráfico de Contribuições de ${encodedName}`,
